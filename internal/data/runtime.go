...
// ErrInvalidRuntimeFormat defines an error that our UnmarshalJSON() method can return if we're unable to parse
...
func (r *Runtime) UnmarshalJSON(val []byte) error {
	...
	// Expected: "X mins" so split the string to isolate the part containing the number.
